...
| `MULTICA_OIDC_ISSUER` | 空 | 通用 SSO 的 OIDC issuer 地址；发现文档从 `<issuer>/.well-known/openid-configuration` 读取 |
| `MULTICA_OIDC_CLIENT_ID` | 空 | OIDC client ID；issuer、client ID、secret 三者齐备时 SSO 才会启用 |
| `MULTICA_OIDC_CLIENT_SECRET` | 空 | OIDC client secret |
| `MULTICA_OIDC_DISPLAY_NAME` | `SSO` | 登录页 SSO 按钮的文案 |
| `MULTICA_OIDC_SCOPES` | `openid email profile` | 向 IdP 请求的 scope |
| `MULTICA_OIDC_REDIRECT_URI` | `${FRONTEND_ORIGIN}/auth/callback` | 在 IdP 上注册的回调地址 |
...
| Composio | `FF_COMPOSIO_MCP_APPS` | Composio 设置入口的产品开关。自托管 compose 默认打开，云端默认关闭 |
