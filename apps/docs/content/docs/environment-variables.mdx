...
| `MULTICA_OIDC_ISSUER` | empty | OIDC issuer URL for generic SSO; discovery is read from `<issuer>/.well-known/openid-configuration` |
| `MULTICA_OIDC_CLIENT_ID` | empty | OIDC client ID; SSO activates only when issuer, client ID, and secret are all set |
| `MULTICA_OIDC_CLIENT_SECRET` | empty | OIDC client secret |
| `MULTICA_OIDC_DISPLAY_NAME` | `SSO` | Label on the login page's SSO button |
| `MULTICA_OIDC_SCOPES` | `openid email profile` | Scopes requested from the IdP |
| `MULTICA_OIDC_REDIRECT_URI` | `${FRONTEND_ORIGIN}/auth/callback` | Redirect URI registered at the IdP |
...
| Composio | `FF_COMPOSIO_MCP_APPS` | Product flag for the Composio settings section. Self-host compose defaults this to true; the managed cloud stays off |
