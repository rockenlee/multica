...
Multica 可以通过任意 OpenID Connect 身份提供方完成登录，例如 Keycloak、Authentik、Azure AD / Entra ID、Okta，或作为企业 IdP 的飞书。

1. 在 IdP 上注册一个使用授权码模式的机密 OAuth client，回调地址填写：
...
issuer 地址必须能通过 `<issuer>/.well-known/openid-configuration` 提供发现文档，且 IdP 必须返回邮箱 claim（默认请求 `openid email profile` scope，可用 `MULTICA_OIDC_SCOPES` 覆盖）。账号按邮箱匹配：之前用邮箱验证码登录的用户切换到 SSO 后仍是同一个账号。下方的注册范围控制同样作用于首次 SSO 登录。配置生效后，登录页会出现 SSO 登录按钮；前端镜像不需要重新构建。
