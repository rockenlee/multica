...
export function applyIssueFilters(
  ...
  const { statusFilters, priorityFilters, assigneeFilters, includeNoAssignee, creatorFilters, projectFilters, includeNoProject, labelFilters, sourceFilters = [], workingOnly } = filters;
  ...
  return issues.filter((issue) => {
    ...
    if (hasSourceFilter) {
      ...
      if (!provider || !sourceFilters.includes(provider)) return false;
    }
