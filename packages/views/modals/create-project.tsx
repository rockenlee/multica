        ...
        <div className="flex items-center gap-1.5 px-4 py-2 shrink-0 flex-wrap">
          ...
          {enablement.feishu && (
            <Popover
              ...
              onOpenChange={(value) => {
                setDrivePopoverOpen(value);
                if (!value) setDriveInput("");
              }}
            >
              <PopoverTrigger
                render={
              ...
              />
              <PopoverContent side="top" align="start" className="w-72 space-y-2 p-2">
                <div className="text-caption font-medium text-muted-foreground">
                ...
                >
                  <input
                    ...
                    className="flex-1 rounded-md border bg-transparent px-2 py-1 text-caption outline-none placeholder:text-muted-foreground"
                  />
                  <Button type="submit" size="sm" variant="ghost" className="h-6 px-2 text-caption" disabled={!driveInput.trim()}>
                    {t(($) => $.create_project.repos_add)}
                  </Button>
                </form>
                {selectedDrives.length > 0 && (
                  <div className="space-y-1 border-t pt-1">
                    {selectedDrives.map((value) => (
                      <div key={value} className="flex items-center gap-2 text-caption">
                        <Cloud className="size-3 text-muted-foreground" />
                        <RepoUrlText url={value} />
                        <button type="button" onClick={() => removeDrive(value)} className="text-muted-foreground hover:text-foreground">
                          <XIcon className="size-3" />
                        </button>
                      </div>
                    ))}
          ...
          {enablement.zentao && (
            <Popover
              ...
              onOpenChange={(value) => {
                setZentaoPopoverOpen(value);
                if (!value) setZentaoInput("");
              }}
            >
              <PopoverTrigger
                render={
              ...
              <PopoverContent side="top" align="start" className="w-72 space-y-2 p-2">
                ...
                >
                  <input
                    type="text"
                    value={zentaoInput}
                    onChange={(event) => setZentaoInput(event.target.value)}
                    placeholder={t(($) => $.create_project.zendao_placeholder)}
                    className="flex-1 rounded-md border bg-transparent px-2 py-1 text-caption outline-none placeholder:text-muted-foreground"
                  />
                  <Button type="submit" size="sm" variant="ghost" className="h-6 px-2 text-caption" disabled={!zentaoInput.trim()}>
                    {t(($) => $.create_project.repos_add)}
                  </Button>
                </form>
                {selectedZentao.length > 0 && (
                  <div className="space-y-1 border-t pt-1">
                    {selectedZentao.map((value) => (
                      <div key={value} className="flex items-center gap-2 text-caption">
                        ...
                        <RepoUrlText url={value} />
                        <button type="button" onClick={() => removeZentao(value)} className="text-muted-foreground hover:text-foreground">
                          <XIcon className="size-3" />
                        </button>
                      </div>
                    ))}
