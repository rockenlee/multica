...
export interface AuthState {
  ...
  loginWithSso: (code: string, redirectUri: string) => Promise<User>;
...
export function createAuthStore(options: AuthStoreOptions) {
  ...
  return create<AuthState>((set) => ({
    ...
    loginWithSso: async (code: string, redirectUri: string) => {
      const { token, user } = await api.ssoLogin(code, redirectUri);
      if (!cookieAuth) {
        storage.setItem("multica_token", token);
        api.setToken(token);
      }
