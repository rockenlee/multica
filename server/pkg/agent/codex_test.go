...
func TestCodexStartOrResumeThreadStartsEphemeral(t *testing.T) {
	t.Parallel()
	...
	wait := drainRPCScript(t, c, fs, []rpcResponse{
		{
			method: "thread/start",
			result: json.RawMessage(`{"thread":{"id":"thr_ephemeral"}}`),
			assertFn: func(t *testing.T, params map[string]any) {
				if params["ephemeral"] != true {
					t.Errorf("ephemeral = %v, want true", params["ephemeral"])
				}
				if params["persistExtendedHistory"] != false {
					t.Errorf("persistExtendedHistory = %v, want false", params["persistExtendedHistory"])
				}
			},
		},
	})
	...
	threadID, resumed, err := c.startOrResumeThread(
		...
		ExecOptions{
			Cwd:             "/work",
			ThreadName:      "ignored for ephemeral threads",
			ResumeSessionID: "thr_prior",
			CodexThreadMode: CodexThreadModeEphemeral,
		},
		slog.Default(),
	)
	if err != nil {
	...
	if threadID != "thr_ephemeral" {
		t.Errorf("threadID = %q, want thr_ephemeral", threadID)
	}
	if resumed {
		t.Error("resumed should be false for ephemeral threads")
	}
	lines := fs.Lines()
	if len(lines) != 1 {
		t.Fatalf("expected only thread/start request, got %d lines: %v", len(lines), lines)
	}
	var req struct {
		Method string `json:"method"`
	}
	...
	if req.Method != "thread/start" {
		t.Fatalf("request method = %q, want thread/start", req.Method)
	}
}
...
func TestCodexExecuteDoesNotPersistEphemeralSessionID(t *testing.T) {
	t.Parallel()
	if runtime.GOOS == "windows" {
		t.Skip("shell-script fixture is POSIX-only")
	}
	...
	fakePath := writeFakeCodexAppServer(t, ""+
		...
		`read line`+"\n"+
		`read line`+"\n"+
		`echo '{"jsonrpc":"2.0","id":2,"result":{"thread":{"id":"thr-ephemeral"}}}'`+"\n"+
		`read line`+"\n"+
		`echo '{"jsonrpc":"2.0","id":3,"result":{}}'`+"\n"+
		`echo '{"jsonrpc":"2.0","method":"turn/started","params":{"threadId":"thr-ephemeral","turn":{"id":"turn-ephemeral"}}}'`+"\n"+
		`echo '{"jsonrpc":"2.0","method":"turn/completed","params":{"threadId":"thr-ephemeral","turn":{"id":"turn-ephemeral","status":"completed"}}}'`+"\n")
	...
	if result.SessionID != "" {
		t.Fatalf("ephemeral thread id must not be persisted, got %q", result.SessionID)
	}
}
