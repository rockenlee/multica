...
func (b *codexBackend) executeOnce(ctx context.Context, prompt string, opts ExecOptions, attempt int) (*Session, error) {
	...
	go func() {
		...
		if err != nil {
			...
			transportFailed := isCodexTransportError(err)
			if timedOut || transportFailed {
				// A timed-out initialize may still complete after the host gives up.
				// A transport failure can also leave a live process that closed stdout.
				// Kill the whole process group before waiting so neither case burns the
				// graceful-shutdown window or leaves detached-stdio descendants behind.
				signalProcessGroup(cmd.Process, syscall.SIGKILL)
				cancel()
			}
		...
		if err != nil {
			...
			if timedOut || transportFailed {
				...
				// the whole process group before waiting. A known transport failure
				// may likewise leave a live process behind after stdout has closed.
				signalProcessGroup(cmd.Process, syscall.SIGKILL)
				cancel()
			}
			drainAndWait() // flush os/exec stderr goroutine before sampling Tail
		...
		// A reader-side transport failure is already terminal; do not spend the
		// graceful window waiting for a process that closed its protocol stream
		// but stayed alive (for example a wrapper that execs a sleeping child).
		if processExitErr != nil {
			signalProcessGroup(cmd.Process, syscall.SIGKILL)
			cancel()
		}
...
func (c *codexClient) startOrResumeThread(ctx context.Context, opts ExecOptions, logger *slog.Logger) (string, bool, error) {
	...
	if threadMode != CodexThreadModeEphemeral {
