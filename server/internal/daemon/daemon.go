...
const defaultAgentVersionProbeTimeout = 20 * time.Second
...
func (d *Daemon) probeBuiltinRuntime(ctx context.Context, name string, entry AgentEntry) (string, string, bool) {
	...
	for attempts < runtimeVersionProbeAttempts {
		...
		attemptCtx, cancelAttempt := context.WithTimeout(ctx, d.effectiveVersionProbeTimeout())
		resolved, _ := d.resolveAgentEntry(attemptCtx, name, entry)
		version, err := detectAgentVersion(attemptCtx, resolved.Path)
...
func (d *Daemon) probeAgentVersion(ctx context.Context, path string) (string, bool, error) {
	probeCtx, cancel := context.WithTimeout(ctx, d.effectiveVersionProbeTimeout())
	defer cancel()
	version, err := detectAgentVersion(probeCtx, path)
	return version, errors.Is(probeCtx.Err(), context.DeadlineExceeded), err
}
