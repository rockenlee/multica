...
func (h *Handler) ListIssues(w http.ResponseWriter, r *http.Request) {
	...
	sourceSystems, ok := parseIssueSourceSystems(w, r.URL.Query().Get("source_systems"))
	if !ok {
		return
	}
	...
	if len(sourceSystems) > 0 {
		where = append(where, fmt.Sprintf("i.metadata->>'source_system' = ANY(%s::text[])", addArg(sourceSystems)))
	}
...
func isIssueSourceSystem(value string) bool {
	switch value {
	case "feishu", "lark", "zentao", "gitlab":
		return true
	default:
		return false
	}
...
func parseIssueSourceSystems(w http.ResponseWriter, raw string) ([]string, bool) {
	values := splitCommaParam(raw)
	for _, value := range values {
		if !isIssueSourceSystem(value) {
			writeError(w, http.StatusBadRequest, "invalid source_systems")
			return nil, false
		}
	}
	return values, true
...
func (h *Handler) ListGroupedIssues(w http.ResponseWriter, r *http.Request) {
	...
	if sourceSystems, ok := parseIssueSourceSystems(w, r.URL.Query().Get("source_systems")); !ok {
		return
	} else if len(sourceSystems) > 0 {
		where = append(where, fmt.Sprintf("i.metadata->>'source_system' = ANY(%s::text[])", addArg(sourceSystems)))
	}
