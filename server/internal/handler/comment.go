...
func (h *Handler) triggerTasksForComment(ctx context.Context, issue db.Issue, comment db.Comment, parentComment *db.Comment, actorType, actorID, originatorUserID, delegationAuthorityUserID string, suppressAgentIDs []pgtype.UUID) []CommentTriggerOutcome {
	...
	enqueued := h.enqueueCommentAgentTriggersWithSource(ctx, issue, comment.ID, comment.SourceTaskID, triggers)
...
func (h *Handler) enqueueCommentAgentTriggers(ctx context.Context, issue db.Issue, triggerCommentID pgtype.UUID, triggers []commentAgentTrigger) map[string]commentEnqueueResult {
	return h.enqueueCommentAgentTriggersWithSource(ctx, issue, triggerCommentID, pgtype.UUID{}, triggers)
}

// enqueueCommentAgentTriggersWithSource adds callback idempotency only when the
// comment is tied to a source agent task. Member comments and other ordinary
// trigger paths retain the official enqueue/coalescing behavior unchanged.
func (h *Handler) enqueueCommentAgentTriggersWithSource(ctx context.Context, issue db.Issue, triggerCommentID, sourceTaskID pgtype.UUID, triggers []commentAgentTrigger) map[string]commentEnqueueResult {
	...
	for _, trigger := range triggers {
		trigger.IdempotencyKey = commentCallbackIdempotencyKey(sourceTaskID, trigger.Agent.ID)
...
func (h *Handler) enqueueSingleCommentTrigger(ctx context.Context, issue db.Issue, triggerCommentID pgtype.UUID, trigger commentAgentTrigger, getEscalationDelay func() time.Duration) error {
	...
	case commentTriggerSourceIssueAssignee:
		if trigger.Squad != nil {
			...
			if trigger.IdempotencyKey != "" {
				_, err = h.TaskService.EnqueueTaskForSquadLeaderIdempotent(ctx, issue, trigger.Agent.ID, trigger.Squad.ID, triggerCommentID, trigger.IdempotencyKey)
			} else {
		...
		var err error
		if trigger.IdempotencyKey != "" {
			_, err = h.TaskService.EnqueueTaskForIssueIdempotent(ctx, issue, triggerCommentID, trigger.IdempotencyKey)
		} else {
	...
	case commentTriggerSourceMentionAgent:
		var err error
		if trigger.IdempotencyKey != "" {
			_, err = h.TaskService.EnqueueTaskForMentionIdempotent(ctx, issue, trigger.Agent.ID, triggerCommentID, trigger.IdempotencyKey)
		} else {
	...
	case commentTriggerSourceThreadParent, commentTriggerSourceConversation:
		...
		if trigger.Source == commentTriggerSourceConversation && trigger.Squad != nil {
			if trigger.IdempotencyKey != "" {
				task, err = h.TaskService.EnqueueTaskForSquadLeaderIdempotent(ctx, issue, trigger.Agent.ID, trigger.Squad.ID, triggerCommentID, trigger.IdempotencyKey)
			} else {
				task, err = h.TaskService.EnqueueTaskForSquadLeader(ctx, issue, trigger.Agent.ID, trigger.Squad.ID, triggerCommentID)
			}
		} else if trigger.IdempotencyKey != "" {
			task, err = h.TaskService.EnqueueTaskForMentionIdempotent(ctx, issue, trigger.Agent.ID, triggerCommentID, trigger.IdempotencyKey)
		} else {
...
func (h *Handler) routeAssignedSquadLeaderFallback(ctx context.Context, issue db.Issue, authorType, authorID string, opts commentTriggerComputeOptions) (commentAgentTrigger, bool) {
	...
	verdict, err := service.AgentReadiness(ctx, h.runtimeLookup(obsmetrics.RuntimeLookupSourceComment), agent)
...
func (h *Handler) retriggerCancelledTaskSurvivors(ctx context.Context, issue db.Issue, cancelled []db.AgentTaskQueue, excludedCommentID pgtype.UUID) {
	...
	for i := range comments {
		...
		if len(scoped) > 0 {
			h.enqueueCommentAgentTriggersWithSource(ctx, issue, comment.ID, comment.SourceTaskID, scoped)
		}
	}
}
