...
func (h *Handler) compileIssueTableQuery(w http.ResponseWriter, r *http.Request, spec issueTableQuerySpec) (issueTableSQL, bool) {
	...
	for _, sourceSystem := range spec.Filters.SourceSystems {
		if !isIssueSourceSystem(sourceSystem) {
			writeError(w, http.StatusBadRequest, "invalid filters.source_systems")
			return issueTableSQL{}, false
		}
	}
	if len(spec.Filters.SourceSystems) > 0 {
		where = append(where, fmt.Sprintf("i.metadata->>'source_system' = ANY(%s::text[])", addArg(sortedUniqueStrings(spec.Filters.SourceSystems))))
	}
